...
def load_model(name: str, checkpoint: str, num_classes: int = 6):
    if name == "resnet50":
...
def main():
    ...
    if args.mode == "single":
        ...
        models = {}
        if args.resnet_ckpt:
            models["ResNet50"] = load_model("resnet50", args.resnet_ckpt).to(device)
        if args.efficientnet_ckpt:
            models["EfficientNet-B3"] = load_model("efficientnet_b3",
                                                    args.efficientnet_ckpt).to(device)
        if args.baseline_ckpt:
            models["Baseline CNN"] = load_model("baseline_cnn",
                                                 args.baseline_ckpt).to(device)
